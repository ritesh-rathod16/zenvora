...
class _FollowersScreenState extends State<FollowersScreen> {
  ...
  Future<void> _fetchUsers() async {
    try {
      final response = await _apiService.get('/users/${widget.username}/followers');
  ...
  Widget _buildUserTile(dynamic user) {
    final username = user['username']?.toString() ?? "Ghost";
    return Container(
      ...
      child: Row(
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.push(context, MaterialPageRoute(builder: (_) => UserProfileScreen(username: username)));
            },
  ...
  Widget _buildEmptyState() {
    return Center(
      child: ListView(
        shrinkWrap: true,
        children: [
          const Icon(Icons.people_outline_rounded, size: 64, color: Colors.white10),
          const SizedBox(height: 16),
          Center(child: Text("No followers found", style: GoogleFonts.poppins(color: Colors.white24))),
        ],
      ),
    );
  }
